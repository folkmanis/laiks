...
import { PresetPowerAppliance, SystemAppliancesService } from '@shared/appliances';
...
const testData: WithId<PresetPowerAppliance>[] = [dishWasher, washer]
    .map((appliance, idx) => ({ ...appliance, localizedNames: { lv: appliance.name }, id: 'id' + idx }));
...
class ApplianceEditTestComponent { @Input() id: string = ''; }

@Component({
    template: `localized-names-component  {{id}}`,
    standalone: true,
})
class LocalizedNamesTestComponent { @Input() id: string = ''; }
...
describe('AppliancesListComponent', () => {
    ...
    beforeEach(async () => {
        ...
        mockService.updateAppliance.and.callFake(async (id, update) => {
            testData$.next(testData$.value.map(value => value.id === id ? { ...value, ...update } : value));
        });
    ...
    it('should display enabled status', async () => {
        const checkboxes = await table.getAllHarnesses(MatCheckboxHarness.with({ selector: '.enabled' }));
        const checked = await Promise.all(checkboxes.map(checkbox => checkbox.isChecked()));
    ...
    it('should navigate to new appliance', async () => {
        ...
        expect(harness.routeNativeElement?.innerHTML)
            .toContain('new-appliance-component');
    });
    ...
    it('should navigate to appliance edit', async () => {
        ...
        const button = await table.getHarness(MatButtonHarness.with({ text: name }));

        await button.click();

        expect(harness.routeNativeElement?.innerHTML)
            .toContain('appliance-edit-component');
        expect(harness.routeNativeElement?.innerHTML)
            .toContain(id);
    });
    ...
    it('should navigate to localized names', async () => {
        ...
        const button = await table.getHarness(MatButtonHarness.with({ selector: '.localized-names' }));

        await button.click();

        expect(harness.routeNativeElement?.innerHTML)
            .toContain('localized-names-component');
        expect(harness.routeNativeElement?.innerHTML)
            .toContain(id);
    });
    ...
    it('should delete appliance', async () => {
        ...
        const button = await table.getHarness(MatButtonHarness.with({ selector: '.delete' }));
