...
import { CollectionReference, Firestore, collection, collectionData, doc, docData, updateDoc, deleteDoc, setDoc } from '@angular/fire/firestore';
...
export class MarketZonesService {
  ...
  private collRef = collection(inject(Firestore), ZONES) as CollectionReference<MarketZone>;
  private docRef = (id: string) => doc(this.collRef, id);

  getZonesFlow(): Observable<WithId<MarketZone>[]> {
    return collectionData(this.collRef, { idField: 'id' }) as Observable<WithId<MarketZone>[]>;
  }
