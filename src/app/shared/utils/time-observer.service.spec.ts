...
describe('TimeObserverService', () => {
    ...
    it('should calculate ms to next minute', () => {
        ...
        expect(service.timeToNextMinute(now)).withContext('at zero').toBe(60 * 1000);
        now.setSeconds(10);
        expect(service.timeToNextMinute(now)).withContext('at minute').toBe(50 * 1000);
    });
    ...
    it('should calculate ms to next day', () => {
        ...
        expect(service.timeToNextDay(now)).withContext('at zero').toBe(24 * 60 * 60 * 1000);
    ...
    it('should emit on each minute', fakeAsync(() => {
        ...
        let minute = new Date();
        service.minuteObserver()
            .pipe(take(3))
            .subscribe(data => minute = data);
        tick(0);
        expect(minute.getTime()).withContext('initial').toBe(start.getTime());
        tick(toNext);
        expect(minute.getMinutes()).withContext('next minute').toBe(start.getMinutes() + 1);
        tick(60 * 1000);
        expect(minute.getMinutes()).withContext('second minute').toBe(start.getMinutes() + 2);
    }));
    ...
    it('should emit on each day', fakeAsync(() => {
        ...
        tick(toNext);
        expect(day.getDate()).withContext('next day').toBe(start.getDate() + 1);
        tick(86400 * 1000);
        expect(day.getDate()).withContext('second day').toBe(start.getDate() + 2);
    }));
